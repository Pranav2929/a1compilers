...
sofia_void startReader(sofia_strg, sofia_strg);
...
sofia_intg main2Reader(sofia_intg argc, sofia_strg* argv) {
	...
	sofia_intg size = 0;
	///sofia_strg output = (char*)malloc(strlen(input) + strlen(SUFFIX_READER) + 1);
    ///if (output == NULL) {  
...
************************************************************
*/
sofia_void startReader(sofia_strg program, sofia_strg input) {
